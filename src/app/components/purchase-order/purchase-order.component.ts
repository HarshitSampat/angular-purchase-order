...
import { PurchaseOrder, VendorDetails } from '../../interfaces/purchase-order.interface';
...
ngOnInit() {
  ...
  this.http.get<PurchaseOrder>('../../assets/purchase-order.json').subscribe((data: any) => {
    this.poOrderDetails = data;

    this.addEmptyRowsIfNeeded();
    
    if(this.poOrderDetails){
      console.log(Object.keys(this.poOrderDetails));
...
countSubTotal ( Discount:number, CGST:number, SGST:number, otherCost:number){
